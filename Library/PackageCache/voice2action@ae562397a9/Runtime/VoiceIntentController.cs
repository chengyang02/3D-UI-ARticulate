    ...
    public class VoiceIntentController : MonoBehaviour
    {
        ...
        private async Task<string> CallWhisper(AudioClip audioClip)
        {
            ...
            try
            {
                ...
                var response = await Utils.openAIClient.AudioEndpoint.CreateTranscriptionAsync(request);
                // var result = response.Text;
                var result = response; // response is already the text string
        ...
        //     string mockResult = "select the tall cube";  // 👈 You can change this to simulate different commands

        //     Debug.Log("[Mocked Whisper] Returning: " + mockResult);
        //     m_OpenAIStatus = true;
        //     return mockResult;
        // }
        ...
        public static async Task<IReadOnlyList<double>> CallEmbedding(string userInput)
        {
            ...
            try
            {
                var chatResponse =
                    ...
                    await Utils.openAIClient.EmbeddingsEndpoint.CreateEmbeddingAsync(userInput, model: Utils.k_EmbeddingModel);
